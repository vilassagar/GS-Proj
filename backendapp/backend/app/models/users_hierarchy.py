...
class Block(Base, TimestampMixin):
    ...
    gram_panchayats: Mapped[List["GramPanchayat"]] = relationship(
        "GramPanchayat", back_populates="block"
    )

    users: Mapped[List["User"]] = relationship("User", back_populates="block")

    __table_args__ = (Index("ix_block_district_name", "district_id", "name"),)
...
class GramPanchayat(Base, TimestampMixin):
    ...
    __table_args__ = (Index("ix_gram_panchayat_block_name", "block_id", "name"),)
