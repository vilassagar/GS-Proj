...
const DocumentUpload = ({
  ...
  console.log("DocumentUpload props:", {
    userId,
    isProfileMode,
    uploadedDocuments,
    documentStatistics,
  ...
  const [uploadedDocIds, setUploadedDocIds] = useState(new Set());
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        ...
        // If uploadedDocuments are passed (from profile page), use them
        if (uploadedDocuments && uploadedDocuments.length > 0) {
          const uploadedIds = new Set(
            uploadedDocuments
              .filter((doc) => doc.submissionStatus === "SUBMITTED")
              .map((doc) => doc.documentTypeId)
          );
          setUploadedDocIds(uploadedIds);
        }
      } catch (error) {
              ...
              <AccordionItem key={category} value={category}>
                ...
                <AccordionContent>
                  <div className="flex flex-wrap gap-4 p-4">
                    {docs.map((docType) => {
                      const isUploaded = uploadedDocIds.has(
        ...
        {documentTypes.length > 0 && (
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              {documentStatistics ? (
                <>
                  <p>एकूण कागदपत्रे: {documentStatistics.totalDocuments}</p>
                  <p>अनिवार्य: {documentStatistics.mandatoryDocuments.total}</p>
                  <p>
                    सबमिट केलेली:{" "}
                    {documentStatistics.mandatoryDocuments.submitted}
                  </p>
                  <p>
                    बाकी: {documentStatistics.mandatoryDocuments.pending}{" "}
                    अनिवार्य
                  </p>
                  <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${documentStatistics.mandatoryDocuments.submissionPercentage}%`,
                      }}
                    ></div>
                  </div>
                  <p className="text-xs mt-1">
                    {documentStatistics.mandatoryDocuments.submissionPercentage}
                    % पूर्ण
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p>अपलोड झालेली: {uploadedDocIds.size}</p>
                  <p>
                    बाकी:{" "}
                    {documentTypes.filter((d) => d.isMandatory).length -
                      Array.from(uploadedDocIds).filter(
                        (id) =>
